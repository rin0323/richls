...
mod completions {
    ...
    fn generate_impl(
        ...
        let parent = destfile.parent().ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidInput, "invalid completion path")
        })?;
    ...
    pub(super) fn generate(outdir: &Path) -> io::Result<()> {
...
pub(crate) fn generate(outdir: &Path) -> std::io::Result<()> {
    #[cfg(debug_assertions)]
    {
